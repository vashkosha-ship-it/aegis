...
async def login(
    ...
    # Проверка rate limit
    allowed, remaining = await login_limiter.check_allowed(ip)
    if not allowed:
        raise HTTPException(
            status_code=429,
...
async def reset_password(
    ...
    # Успешный сброс подтверждает почту.
    #
    # Код приходит на тот же адрес, что и код регистрации, и доказывает ровно
    # то же самое — что человек этим ящиком владеет. Раньше здесь выдавались
    # токены без всякой оглядки на is_verified: неподтверждённый аккаунт
    # входил через восстановление пароля, хотя обычный вход ему отказывал.
    # Оставался пользователь, который одновременно вошёл в систему и числится
    # неподтверждённым — состояние, которого по замыслу быть не может.
    if not user.is_verified:
        logger.info(
            "Пользователь %s подтверждён через сброс пароля: код с того же "
            "адреса доказывает владение ящиком", user.id,
        )
        user.is_verified = True
    user.verify_code = None
    user.verify_expires = None
