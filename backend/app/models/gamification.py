...
async def check_and_award_achievements(
    ...
    # Выдача одним запросом с ON CONFLICT DO NOTHING.
    #
    # Между чтением списка полученных достижений и вставкой проходит время, и
    # за него параллельный запрос может выдать ту же награду. Дубля не будет —
    # мешает уникальный индекс uq_user_achievement, — но прежняя вставка через
    # db.add() падала бы с IntegrityError на коммите. Причём коммит делает
    # вызывающий код, так что откатывалась вся его транзакция: пользователь
    # терял сданный тест из-за того, что достижение выдали дважды.
    #
    # RETURNING возвращает только реально вставленные строки, поэтому список
    # новых кодов остаётся честным: тот, кто проиграл гонку, не сообщит о
    # награде, которую выдал не он.
    rows = (
        await db.execute(
            select(Achievement.id, Achievement.code).where(
                Achievement.code.in_(new_codes)
            )
        )
    ).all()
    if not rows:
        # Кодов нет в каталоге — достижения не засеяны. Молча пропускаем, как
    ...
    code_by_id = {achievement_id: code for achievement_id, code in rows}
    # Порядок сохраняем исходный: по нему фронт показывает уведомления.
    return [
        code for code in new_codes
        if any(code_by_id.get(i) == code for i in granted_ids)
    ]