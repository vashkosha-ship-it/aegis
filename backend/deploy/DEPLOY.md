...
После проверки пароля административная учётная запись всегда запрашивает
одноразовый код из письма. Код отправляется на email пользователя; для старой
учётной записи без email используется `ADMIN_NOTIFY_EMAIL`. При первом
успешном входе интерфейс один раз показывает десять recovery-кодов — сохраните
их в менеджере паролей. В БД находятся только HMAC этих кодов.

Миграция `admin_mfa_audit` отзывает все admin-сессии, созданные до включения
второго фактора. Поэтому до деплоя проверьте SMTP и наличие email:

```bash
grep -E '^(SMTP_HOST|ADMIN_NOTIFY_EMAIL)=' /opt/aegis/backend/.env
```
...
`/var/backups/aegis`, записывает SHA-256 и по умолчанию хранит одну последнюю
полную копию. Предыдущая удаляется только после успешного создания новой. Если
файлы книг не менялись, следующий запуск создаёт hard link на проверенный
архив storage, поэтому во время ротации одни и те же PDF/EPUB не занимают
место повторно. Перед новым полным архивом скрипт проверяет свободное место и
оставляет минимум 1 ГБ для системы. Число локальных копий можно увеличить
через `AEGIS_BACKUP_KEEP_COUNT`, но длительную историю лучше хранить на другом
сервере или в объектном хранилище. Проверка:
