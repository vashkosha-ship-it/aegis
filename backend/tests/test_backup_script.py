...
def test_s3_backup_contains_database_and_backend_marker(tmp_path):
    fake_bin = tmp_path / "bin"
    fake_bin.mkdir()
    fake_pg_dump = fake_bin / "pg_dump"
    fake_pg_dump.write_text(
    ...
    completed = list(backup_dir.glob("????????T??????Z"))
    assert len(completed) == 1
    backup = completed[0]
    assert (backup / "database.dump").read_bytes() == b"test-dump"
    assert (backup / "STORAGE_BACKEND").read_text(encoding="utf-8") == "s3\n"
    assert not (backup / "storage.tar.gz").exists()
    assert "database dump only" in result.stdout
